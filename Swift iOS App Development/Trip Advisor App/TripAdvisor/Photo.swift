...
class Photo{
    ...
    func saveData(city: City, event: Event, completed: @escaping (Bool) -> ()){
        ...
        let storageRef = storage.reference().child(city.documentID).child(self.documentUUID)
        ...
        uploadTask.observe(.success) { (snapshot) in
            ...
            let ref = db.collection("cities").document(city.documentID).collection("events").document(event.documentID).collection("photos").document(self.documentUUID)
            ref.setData(dataToSave){(error) in
                if let error = error{
                    print("error updateding doc \(self.documentUUID) in city \(city.documentID) \(error.localizedDescription)")
                    completed(false)
                }
                else{
        ...
        uploadTask.observe(.failure) { (snapshot) in
            if let error = snapshot.error{
                print("ERROR UPLOAD TASK FOR FILE \(self.documentUUID) failed in spot \(city.documentID)")
            }
            return completed(false)
        }
